...
lm_profit_final = lm(X9Profit ~ factor(X9Age) + factor(X9Inc) + factor(X9District) + 
                     ...
                     data = training.data)
# summary(lm_profit_final) # Please see attachments for the summary of retention model
```
...
# comparison.table.profit (Please see attachment "Comparison Table for Profit Models")
```
...
lm_retention_Tenure = lm(retention ~ factor(X9Age) + factor(X9Inc) + X9Online * factor(X9Age)
                         ...
                         data = training.data)
# Please see attachments for the summary of retention model
```
...
# comparison.table.retention (Please see attachment "Comparison Table for Profit Models")
```
...
Prediction_Retention_2000 = predict(lm_retention_Online, Database2000)
